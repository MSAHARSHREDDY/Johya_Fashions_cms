...
export default function CustomerForm({ customer, onSubmit, isLoading }: CustomerFormProps) {
  ...
  const { computePoints, settings } = usePointSettings();
  const priceValue = form.watch('price');

  useEffect(() => {
    // Dynamic rewards calculation when price or point settings change
    if (!customer) {
      const num = Number(priceValue);
      if (priceValue !== '' && !isNaN(num)) {
        form.setValue('rewards', computePoints(num));
      } else if (priceValue === '') {
        form.setValue('rewards', '' as any);
      }
    }
  }, [priceValue, customer, form, computePoints, settings]);
  ...
  const onFormSubmit = (values: z.infer<typeof formSchema>) => {
    const numPrice = Number(values.price) || 0;
    const computedReward = computePoints(numPrice);
    const finalRewards = (values.rewards !== '' && values.rewards !== undefined && values.rewards !== null)
      ? Number(values.rewards)
      : computedReward;
            ...
            render={({ field }) => (
              <FormItem>
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-[#B08D57] font-semibold">
                    Rate: ₹{(settings.minAmount || 1000).toLocaleString()} = {settings.basePoints !== undefined ? settings.basePoints : (settings.pointsPerIncrement ?? 50)} pts
                  </span>
                </div>
                <FormControl>
