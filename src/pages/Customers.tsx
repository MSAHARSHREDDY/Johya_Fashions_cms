...
import { usePointSettings } from '@/hooks/use-point-settings';
...
export default function Customers({ onView, globalSearch = '' }: CustomersProps) {
  ...
  const { computePoints } = usePointSettings();
  ...
  const handleDownloadExcel = async () => {
    setIsExporting(true);
    try {
      // Fetch all customer records matching current search and filters
      ...
      const list: Customer[] = (res.data && res.data.length > 0) ? res.data : customers;

      if (!list || list.length === 0) {
        toast.error('No customer records found to download.');
        return;
      }

      // Format customer information with Name and Phone Number prominently
      const excelRows = list.map((c: Customer, idx: number) => ({
        'S.No': idx + 1,
        'Customer Name': c.name || 'N/A',
        'Phone Number': String(c.phoneNumber || ''),
        'Total Spent (₹)': c.price ?? 0,
        'Offer / Reward Points': c.rewards ?? 0,
        'Categories': Array.isArray(c.categories) ? c.categories.join(', ') : (c.categories || ''),
        'Visits': c.visits ?? 1,
        'Last Purchase Date': c.lastPurchaseDate ? formatIST(new Date(c.lastPurchaseDate), 'dd MMM yyyy') : 'N/A',
      }));

      const worksheet = XLSX.utils.json_to_sheet(excelRows);

      // Auto-fit column widths for crisp Excel display
      worksheet['!cols'] = [
        { wch: 6 },  // S.No
        { wch: 28 }, // Customer Name
        { wch: 20 }, // Phone Number
        { wch: 18 }, // Total Spent (₹)
        { wch: 22 }, // Offer / Reward Points
        { wch: 26 }, // Categories
        { wch: 10 }, // Visits
      ...
      XLSX.writeFile(workbook, `Johya_Customers_${dateStr}.xlsx`);
      toast.success(`Downloaded ${list.length} customer records in Excel (Name & Phone Number).`);
    } catch (err: any) {
  ...
  const downloadSingleCustomer = (c: Customer) => {
    try {
      const excelRows = [{
        'S.No': 1,
        'Customer Name': c.name || 'N/A',
      ...
      const safeName = (c.name || 'Customer').replace(/[^a-zA-Z0-9]/g, '_');
      XLSX.writeFile(workbook, `${safeName}_Info.xlsx`);
      toast.success(`Exported ${c.name}'s information to Excel.`);
    } catch (err: any) {
      toast.error('Failed to export customer info: ' + (err.message || ''));
    }
  };
  ...
  useEffect(() => {
    ...
    if (!isNaN(val)) {
      setPurchaseRewards(computePoints(val));
    } else {
      setPurchaseRewards(0);
    }
  }, [purchaseAmount, computePoints]);
  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      <Card className="flex-grow bg-white rounded-3xl shadow-lg border border-[#F2F2F2] overflow-hidden flex flex-col">
        <div className="p-6 border-b border-[#F2F2F2] flex flex-col sm:flex-row justify-between sm:items-center gap-4 bg-white">
          <div className="flex items-center gap-3">
            <h2 className="font-serif text-xl">Customer Directory</h2>
            {pagination?.total !== undefined && (
              <Badge variant="outline" className="text-[10px] tracking-wider text-black/60 border-[#E5E5E5] px-2.5 py-0.5 font-normal">
                {pagination.total} {pagination.total === 1 ? 'Customer' : 'Customers'}
              </Badge>
            )}
          </div>
          <div className="flex flex-wrap items-center gap-2">
            {/* Separate Icon to Download Users Info in Excel */}
            <Button
              ...
              variant="outline"
              size="sm"
              title="Download customer names and phone numbers in Excel (.xlsx)"
              className="bg-white hover:bg-[#FDFBF7] text-black border-[#E5E5E5] hover:border-[#107C41] rounded-full text-[11px] uppercase tracking-wider font-bold h-9 px-3.5 gap-2 transition-all shadow-xs cursor-pointer"
            >
              {isExporting ? (
                <Loader2 className="h-4 w-4 animate-spin text-[#107C41]" />
              ) : (
            ...
            <Select value={String(limit)} onValueChange={(v) => setLimit(Number(v))}>
              <SelectTrigger className="w-[120px] bg-white border border-[#E5E5E5] rounded-full text-[11px] uppercase tracking-wider font-bold h-9 px-3">
            ...
            <Popover>
              <PopoverTrigger render={<Button variant="outline" className="bg-white border-[#E5E5E5] rounded-full text-[11px] uppercase tracking-wider font-bold h-9 px-3 gap-2" />}>
                      ...
                      <TableCell className="px-6 py-4 text-right flex items-center justify-end gap-2">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="rounded-xl border-[#F2F2F2] shadow-xl">
                            ...
                            <DropdownMenuItem onClick={() => downloadSingleCustomer(customer)} className="text-[11px] uppercase tracking-widest font-bold">
                              <FileSpreadsheet className="mr-2 h-3 w-3 text-[#107C41]" /> Download Excel
                            </DropdownMenuItem>
                    ...
                    <div className="flex justify-between items-center pt-1">
                      ...
                      <div className="flex gap-2">
                        <Button 
                          variant="ghost" 
                          size="sm" 
                          onClick={() => downloadSingleCustomer(customer)} 
                          title="Download Excel"
                          className="h-8 w-8 text-[#107C41] p-0 rounded-full hover:bg-[#F2F8F4]"
                        >
