  ...
  const handleExportExcel = () => {
    ...
    try {
      const excelRows = [{
        ...
        'Phone Number': String(customer.phoneNumber || ''),
        'Total Spent (₹)': customer.price ?? 0,
        'Offer / Reward Points': customer.rewards ?? 0,
        'Categories': Array.isArray(customer.categories) ? customer.categories.join(', ') : (customer.categories || ''),
        'Visits': customer.visits ?? 1,
        'Last Purchase Date': customer.lastPurchaseDate ? formatIST(new Date(customer.lastPurchaseDate), 'dd MMM yyyy') : 'N/A',
      }];
      ...
      toast.success(`Downloaded ${customer.name}'s information in Excel.`);
    } catch (err: any) {
      toast.error('Failed to export customer Excel file: ' + (err.message || ''));
    }
  };
  ...
  return (
    <div className="space-y-8 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Button variant="ghost" onClick={onBack} className="gap-2 text-[11px] uppercase tracking-widest font-bold opacity-60 hover:opacity-100 self-start">
        ...
        <div className="flex items-center gap-3">
          <Button 
            ...
            variant="outline" 
            className="rounded-full px-4 text-[11px] uppercase tracking-widest font-bold border-[#E5E5E5] hover:border-[#107C41] gap-2"
          >
            <FileSpreadsheet className="h-3.5 w-3.5 text-[#107C41]" /> Download Excel
          </Button>
          <Button onClick={onEdit} variant="outline" className="rounded-full px-8 text-[11px] uppercase tracking-widest font-bold border-[#F2F2F2] hover:bg-white">
            Edit Profile
          </Button>
        </div>
      </div>
