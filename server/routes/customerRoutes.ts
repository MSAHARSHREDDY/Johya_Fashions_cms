...
router.post('/recalculate-points', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Fetch stored settings from DB as fallback
    const dbSetting = await Setting.findOne({ key: 'point_settings' });
    const spendAmount = Math.max(1, Number(minAmount ?? dbSetting?.minAmount ?? 1000));
    const pointsEarned = Math.max(0, Number(basePoints ?? dbSetting?.basePoints ?? 50));
...
router.post('/:id/purchases', async (req: Request, res: Response) => {
  ...
  try {
    ...
    let earned = 0;
    if (rewardsEarned !== undefined && rewardsEarned !== null) {
      earned = Number(rewardsEarned) || 0;
    } else {
      const dbSetting = await Setting.findOne({ key: 'point_settings' });
      const spendAmount = Math.max(1, Number(dbSetting?.minAmount ?? 1000));
      const pointsEarned = Math.max(0, Number(dbSetting?.basePoints ?? 50));
