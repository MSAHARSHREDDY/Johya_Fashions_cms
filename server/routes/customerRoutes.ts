...
import { getActivePointSettings } from './settingRoutes.js';
...
router.get('/stats', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Start of today in IST (Asia/Kolkata)
    const istOffsetMs = 5.5 * 60 * 60 * 1000;
    const nowIST = new Date(now.getTime() + istOffsetMs);
    const startOfTodayUTC = new Date(Date.UTC(nowIST.getUTCFullYear(), nowIST.getUTCMonth(), nowIST.getUTCDate(), 0, 0, 0) - istOffsetMs);
    ...
    const [
      ...
      Customer.aggregate([
        { $unwind: '$purchaseHistory' },
        { $match: { 'purchaseHistory.date': { $gte: startOfTodayUTC } } },
        {
          $group: {
            _id: { hour: { $hour: { date: '$purchaseHistory.date', timezone: 'Asia/Kolkata' } } },
            revenue: { $sum: '$purchaseHistory.amount' }
          }
        },
        { $sort: { '_id.hour': 1 } }
    ...
    const stats = {
      ...
      todayHourlyData: todayHourlyAgg.map(h => ({
        hour: h._id.hour,
        revenue: h.revenue
      })),
      customerGrowth,
...
router.post('/recalculate-points', async (req: Request, res: Response) => {
  ...
  try {
    const { minAmount, basePoints } = req.body;
    
    // Fetch active settings as fallback
    const activeSetting = await getActivePointSettings();
    const spendAmount = Math.max(1, Number(minAmount ?? activeSetting?.minAmount ?? 1000));
    const pointsEarned = Math.max(0, Number(basePoints ?? activeSetting?.basePoints ?? 50));
    ...
    for (const customer of customers) {
      ...
      for (const purchase of customer.purchaseHistory) {
        ...
        if (purchase.amount > 0) {
          earnedThisPurchase = Math.floor(purchase.amount * (pointsEarned / spendAmount));
        }
...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (rewards !== undefined && rewards !== null && rewards !== '') {
      finalRewards = Number(rewards) || 0;
    } else {
      const activeSetting = await getActivePointSettings();
      const spendAmount = Math.max(1, Number(activeSetting.minAmount || 1000));
      const pointsEarned = Math.max(0, Number(activeSetting.basePoints ?? activeSetting.pointsPerIncrement ?? 50));
      if (numPrice > 0) {
        finalRewards = Math.floor(numPrice * (pointsEarned / spendAmount));
      }
    }
...
router.post('/:id/purchases', async (req: Request, res: Response) => {
  ...
  try {
    ...
    let earned = 0;
    if (rewardsEarned !== undefined && rewardsEarned !== null && rewardsEarned !== '') {
      earned = Number(rewardsEarned) || 0;
    } else {
      const activeSetting = await getActivePointSettings();
      const spendAmount = Math.max(1, Number(activeSetting?.minAmount ?? 1000));
      const pointsEarned = Math.max(0, Number(activeSetting?.basePoints ?? activeSetting?.pointsPerIncrement ?? 50));
      ...
      if (purchaseAmount > 0) {
        earned = Math.floor(purchaseAmount * (pointsEarned / spendAmount));
      }
    }
